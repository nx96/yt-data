...
from .channel import DEFAULT_SHOW, KEYWORDS, SHOWS
...
def find_keywords_in_title(title):
    for key in KEYWORDS:
        if title.find(key) != -1:
            return True
...
def add_columns_to_df(username: str) -> Optional[str]:
    ...
    for index in df.index:
        ...
        title = title[:(max_len+10)]
        df.at[index, "show"] = DEFAULT_SHOW
        df.at[index, "show_id"] = DEFAULT_SHOW
        for show in SHOWS:
            if title.find(show["id"]) != -1 and df.at[index, "duration_seconds"] > 1800 and not find_keywords_in_title(title):
